...
## Part 2.5：LLM是如何训练出来的？（Post-Training方法）
...
Stage 2: SFT (监督微调)  
    ↓ 高质量对话数据，学习对话格式
Stage 3: Post-Training (后训练对齐)
    ↓ RLHF/DPO/GRPO/RLAIF，对齐人类偏好
```
...
| 时期 | 方法 | 代表模型 | 特点 |
|------|------|----------|------|
| 2022-2023 | RLHF (PPO) | ChatGPT, Claude 1-2 | 需要Reward Model + PPO训练 |
| 2024 | DPO | Llama 3, Qwen 2 | 直接优化，无需Reward Model |
| 2025 | GRPO | DeepSeek R1 | 可验证奖励，适合推理任务 |
| 2026 | RLAIF | Claude 4, Kimi K2 | 用AI替代人工反馈 |
...
| **DPO** | ❌ 不需要 | 中 | 快速迭代优化 |
| **GRPO** | ❌ 不需要 | 中 | 推理/数学/代码 |
| **RLAIF** | ❌ 不需要 | 中 | 大规模低成本训练 |

### MBA视角：商业含义

1. **门槛在下降**：从RLHF到DPO/GRPO，对齐训练成本大幅降低
2. **小团队也能做**：DeepSeek用GRPO训练出顶级推理模型
3. **数据依赖减少**：RLAIF用AI生成反馈，不再依赖大量人工标注
4. **垂直领域机会**：企业可以在特定领域做SFT+DPO微调

> **创业启示**：2024年之后，中小团队用DPO/GRPO也能训练出有竞争力的垂直模型。
